echo "Add the audio menu's rofi stub"

# ~/.config/rofi is copied once at install and never touched again, so a menu
# added later has no stub on a machine that already exists and SUPER + S would
...
STUB_DIR="$HOME/.config/rofi/audio"
...
mkdir -p "$STUB_DIR"
cat >"$STUB" <<'STUBEOF'
@import "hyprsimple/audio/style.rasi"
...
echo "  Added $STUB, so SUPER + S opens the menu in your theme's colours."
