echo "Add the screenshot menu's rofi stub"

# ~/.config/rofi is copied once at install and never touched again, so a menu
# added later has no stub on a machine that already exists and Print would
...
STUB_DIR="$HOME/.config/rofi/screenshot"
...
mkdir -p "$STUB_DIR"
cat >"$STUB" <<'STUBEOF'
@import "hyprsimple/screenshot/style.rasi"
...
echo "  Added $STUB, so Print opens the menu in your theme's colours."
