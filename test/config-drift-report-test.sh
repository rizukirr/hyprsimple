...
ship() { # ship <path> <content>: publish a new version from the seed
  # A path in a directory the seed does not have yet would otherwise fail here
  # and leave the case testing nothing, which is how the generated-file check
  # first passed against a file that was never shipped.
  mkdir -p "$(dirname "$SEED/$1")"
  printf '%s\n' "$2" >"$SEED/$1"
  git -C "$SEED" add -A && git -C "$SEED" commit -qm "change $1"
  git -C "$SEED" push -q "$ORIGIN" main
  if [[ ! -f $SEED/$1 ]]; then
    printf 'not ok - ship did not create %s, so this case tests nothing\n' "$1" >&2
    exit 1
  fi
}
...
# A file hyprsimple writes itself always differs from the shipped copy, and the
# difference is this project's own doing rather than the user's. Reporting it
# was not merely noise: the command offered alongside would have replaced the
# active theme's wallpaper path and lock screen colours with the shipped
# defaults. Measured on a live install before the fix, the report named
# hypr/hyprpaper.conf and hypr/theme-hyprlock.conf, and following it would have
# swapped a rosepine wallpaper path for deep-sea's.
new_case generated
mkdir -p "$FAKE_HOME/.config/hypr"
printf 'path = my-own-theme\n' >"$FAKE_HOME/.config/hypr/hyprpaper.conf"
printf 'MINE\n' >"$FAKE_HOME/.config/starship.toml"
ship .config/hypr/hyprpaper.conf "# Do not edit this file. It is rewritten in full.
path = shipped-default"
ship .config/starship.toml VGEN
run_update
check "a file whose header says it is generated is not reported" \
  "$(grep -c 'hyprpaper.conf' "$CASE/out")" "0"
# The same run has to still report an ordinary file, or the check above would
# pass on a report that named nothing at all.
check "while an ordinary config in the same update still is" \
  "$(grep -c 'refresh-config.sh starship.toml' "$CASE/out")" "1"

new_case generated_marker_absent
mkdir -p "$FAKE_HOME/.config/hypr"
printf 'path = my-own-theme\n' >"$FAKE_HOME/.config/hypr/hyprpaper.conf"
ship .config/hypr/hyprpaper.conf "path = shipped-default"
run_update
check "the same file without that header is reported, so the header is what decides" \
  "$(grep -c 'hyprpaper.conf' "$CASE/out")" "1"
